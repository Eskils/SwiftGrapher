...
public class GutterView: NSView {
    ...
    @Invalidating(.display)
    var lineNumberIndicesWithError: Set<Int> = [] 
//    {
//        didSet {
//            needsDisplay = true
//            display()
...
//    }
    ...
    private func drawLineNumbers(_ context: CGContext) {
        ...
        for linePosition in textView.layoutManager.visibleLines() {
            ...
            drawErrorsOnLineNumbers(context, linePosition: linePosition)
        }
        context.restoreGState()
    }
    
    private func drawErrorsOnLineNumbers(_ context: CGContext, linePosition: TextLineStorage<TextLine>.TextLinePosition) {
        guard
            let textView = textView,
            lineNumberIndicesWithError.contains(linePosition.index)
        else { return }
        
        let circleDiameter = 12.0
        let circleColor = NSColor.systemRed
            
        let fragment: LineFragment? = linePosition.data.lineFragments.first?.data
        let yPos = linePosition.yPos + (fragment?.heightDifference ?? 0) / 2 + (linePosition.height - circleDiameter) / 2
        let xPos = 8.0
        
        context.setFillColor(circleColor.cgColor)
        context.addEllipse(in: CGRect(x: xPos, y: yPos, width: circleDiameter, height: circleDiameter))
        context.fillPath()
    }
